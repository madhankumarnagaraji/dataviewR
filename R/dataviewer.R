...
dataviewer <- function(data = NULL) {
  ...
  shiny::shinyApp(
    ...
    server = function(input, output, session) {
      ...
      shiny::observe({
        shiny::updateTextInput(session, "filter", label = NULL, value = "", placeholder = "Enter a filter condition")
      })
      ...
      generated_code <- shiny::reactive({
        code_lines <- c(
          ...
          "# Change the name of the dataframe if selected from the packages",
          "library(dplyr)",
      ...
      shiny::observe({
        att_cols <- shiny::reactive({
          purrr::map(data2(), attributes) %>% unlist() %>% tibble::enframe() %>%
            dplyr::mutate(colname = sapply(strsplit(.data$name, ".", fixed = TRUE), "[", 1),
                          att = sapply(strsplit(.data$name, ".", fixed = TRUE), "[", 2)) %>%
            dplyr::select(colname, att, value)
        })

        class_df <- shiny::reactive({
          labelled::generate_dictionary(data2()) %>% dplyr::mutate(colname = .data$variable) %>%
