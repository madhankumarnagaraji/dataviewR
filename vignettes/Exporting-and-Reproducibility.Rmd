...
---
# Exporting & reproducing the data to work further
...
To manage and retrieve information about active `dataviewR` sessions, the following helper functions are available.

- `list_dataviewers` → Lists all active `dataviewR` sessions.
- `stop_dataviewer` → To stop the single `dataviewR` session.
- `stop_all_dataviewers`→ To stop all active `dataviewR` session.

## 1. To know the session information
...
## 2. To stop `dataviewR` sessions

To stop the single dataviewR, user can use `stop_dataviewer` by providing the id of the session. 
