...
test_that("App Integration: Filter UI class toggle", {
  skip_on_cran()

  # Launch app with dataset
  app <- dataviewer(mtcars, background = FALSE)
  app_driver <- AppDriver$new(app, name = "filter_ui_test")
  app_driver$wait_for_idle()

  # Get the active tab (mtcars) ID dynamically generated by the module
  values <- app_driver$get_values()
  mtcars_tab_id <- values$input$opt

  # Construct the module-namespaced IDs
  filter_input_id <- paste0(mtcars_tab_id, "-filter")
  submit_btn_id <- paste0(mtcars_tab_id, "-submit")
  label_id <- paste0(mtcars_tab_id, "-label_filter")

  # 1. Test Valid Filter
  app_driver$set_inputs(!!filter_input_id := "cyl == 4")
  app_driver$click(submit_btn_id)
  app_driver$wait_for_idle()

  # Verify the '.filter-active' class is added
  has_active_class <- app_driver$get_js(sprintf(
    "document.getElementById('%s').classList.contains('filter-active')",
    label_id
  ))
  expect_true(has_active_class)

  # 2. Test Invalid Filter (Type mismatch: assigning a character to a numeric column)
  app_driver$set_inputs(!!filter_input_id := "cyl == 'four'")
  app_driver$click(submit_btn_id)
  app_driver$wait_for_idle()

  # Verify the '.filter-active' class is removed after the error resets the state
  has_active_class_after_error <- app_driver$get_js(sprintf(
    "document.getElementById('%s').classList.contains('filter-active')",
    label_id
  ))
  expect_false(has_active_class_after_error)
